...
class Auth extends Controller {
    ...
    public function forget($data): void {
        ...
        $email->add("Recupere sua senha | ".site("name"), $this->view->render("emails/recover", [
            ...
            ]), "{$user->Username}", "{$user->Email}")->send() ? flash("success", "Email de recuperação enviado!") : flash("error", "Erro ao enviar email!");
   ...
   public function contact($data) {
        ...
        $assunto = filter_var($data["assunto"], FILTER_DEFAULT);
        $message = filter_var($data["message"], FILTER_DEFAULT);
        $anon = (isset($data["anon"])) ? 1 : 0;


        if(!getSessionUser()) {
            echo $this->ajaxResponse("message", [
                "type" => "error",
                "message" => "Por favor logue-se para enviar uma mensagem!"
            ]);
        ...
        if(strlen($assunto) > 50) {
            echo $this->ajaxResponse("message", [
                "type" => "error",
                "message" => "Assunto muito grande!"
            ]);
        ...
        if(getUserMessages(getSessionUser()->Id, true) >= 3) {
            echo $this->ajaxResponse("message", [
                "type" => "error",
                "message" => "Você já atingiu o limite de 3 mensagens enviadas... Você não podera enviar mais mensagens até que um administrador veja!"
            ]);
        ...
        if(!$obj->save()) {
            echo $this->ajaxResponse("message", [
                "type" => 'error',
                "message" => 'Erro ao enviar mensagem!'.$anon
            ]);
            return;
        }
